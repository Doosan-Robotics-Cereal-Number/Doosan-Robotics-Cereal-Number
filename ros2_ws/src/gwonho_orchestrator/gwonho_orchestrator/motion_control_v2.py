    ...
    def _late_init():
        ...
        try:
            from DSR_ROBOT2 import (
                movej, movel, movesx, posj, posx, wait,
        ...
        def step_give_cereal_a (params, gh, check_cancel):
            ...
            #on compliance
            #fb(gh, 0.45, "give_cereal_a8"); movel(q_give_cereal_a8, 200*sp, 200*sp); check_cancel() #비동기 
            #get_tool_force_Y
            #on compliance off
            fb(gh, 0.5, "release"); wait(0.2); check_cancel()
            #fb(gh, 0.55, "give_cereal_a9"); movel(q_give_cereal_a9, 200*sp, 200*sp); check_cancel() #tool 기준으로 살짝 앞으로
            fb(gh, 0.6, "grab"); wait(0.2); check_cancel()

            #저울값이 일정 값 이상이 될때 까지 반복 수행 
            # fb(gh, 0.65, "give_cereal_a10"); movel(q_give_cereal_a10, 200*sp, 200*sp); check_cancel() #tool 기준으로 살짝 앞으로
            # fb(gh, 0.75, "give_cereal_a11"); movel(q_give_cereal_a11, 200*sp, 200*sp); check_cancel() #tool 기준으로 살짝 앞으로
            fb(gh, 0.8, "relese"); wait(0.2); check_cancel()
        ...
        def step_give_cereal_b (params, gh, check_cancel):
            ...
            #on compliance
            #fb(gh, 0.3, "give_cereal_b3"); movel(q_give_cereal_b3, 50*sp, 50*sp); check_cancel() #비동기 
            #get_tool_force_Y
            #on compliance off
            fb(gh, 0.4, "relese"); wait(0.2); check_cancel()
            #fb(gh, 0.55, "give_cereal_b4"); movel(q_give_cereal_b4, 200*sp, 200*sp); check_cancel() #tool 기준으로 살짝 앞으로
            fb(gh, 0.5, "grab"); wait(0.2); check_cancel()

            #저울값이 일정 값 이상이 될때 까지 반복 수행 
            # fb(gh, 0.6, "give_cereal_b5"); movel(q_give_cereal_b10, 200*sp, 200*sp); check_cancel() #tool 기준으로 살짝 앞으로
            # fb(gh, 0.7, "give_cereal_b6"); movel(q_give_cereal_b11, 200*sp, 200*sp); check_cancel() #tool 기준으로 살짝 앞으로
